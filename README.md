...
# :page_facing_up: Detail explaination
...
## 연구 결과
색상에 대해 적게 변동하며 기존의 물체 색상으로 잘 렌더링을 하는 것을 확인할 수 있다. 네트워크 내부에서 색상 변동에 대하여 각 이미지 별로 임베딩 공간을 학습하고, 학습한 임베딩 공간을 이용하여 각 포인트의 색상 변동값을 계산하기 때문에, 기존의 NeRF에 비해 색상 변동에 대하여 영향을 적게 받으며 결과를 도출하는 것을 확인할 수 있다. 또한, 두번째 네트워크에서 변동값과 물체의 위치값을 이용하여 최종적인 색상 값을 도출해내므로, 해당 네트워크는 물체의 기존 색상에 대한 학습이 가능하다. 따라서 어떠한 이미지를 입력하였을 때, 해당 이미지에서 기준 이미지 대비 색상 변화량을 도출하고, 이를 이용하여 최종 색상을 도출했기 때문에 우리가 원하는 조건에 해당하는 물체 생성이 가능하다. 추가적으로 기존의 NeRF[1]와 비교하였을 때, 물체를 더욱 세밀하게 생성한 것을 확인할 수 있다. 특히Fig 11과 같이 레고의 팔 부분이나, 여러 부분이 겹쳐있는 복잡한 부분 같은 경우, NeRF는 정확하게 물체를 생성하지 못한 것을 확인할 수 있다. 해당 부분의 경우 색상 변동이 없는 이미지를 이용할 때에도 다른 부분에 비해 정확하게 생성하는 것이 어렵지만, 색상 변동으로 인해 네트워크 학습에 더욱 큰 혼란을 주어 기존보다 더 흐릿하게 결과를 도출하게 된다. 추가적으로 기존의 해당 문제점을 연구한 NeRF-W의 결과와 본 연구 결과에 대한 비교를 진행하였다. 결과 사진을 통해서 볼 수 있듯이, 두 모델간 각 결과의 큰 차이는 보이지 않는 것을 확인할 수 있다. 하지만, 이때 NeRF-W의 경우 모델의 복잡도가 본 논문에서 구현한 모델에 비해 훨씬 많은 네트워크 구조를 사용하기 때문에 학습을 하는 데에 있어서 더욱 오래 걸린다는 단점이 있다. 또한, 세부부분에 있어서 본 연구에서 제안한 모델이 NeRF-W보다 더욱 정밀하게 물체를 생성하는 것을 확인할 수 있다. 이는 물체의 색상 변동값과 기존의 색상에 대한 학습을 동일한 네트워크에서 이루어지기 때문에 발생하는 문제점이라고 해석하였다.
